...
const get_all_keranjang = async (req, res) => { try {
...
                            model: tbl_Wisata,
                            as: "tiket_detail_as",
                            attributes: ["id_wisata", "nama_destinasi", "harga_tiket", "sampul_destinasi"],
                            required: false,
                        },
                    ]
                });
...
                    attributes: ["id_paket_wisata", "nama_paket_wisata", "sampul_paket_wisata"]
                });
...
                            attributes: ["id_paket_wisata", "nama_paket_wisata", "harga_paket_wisata", "sampul_paket_wisata"],
                            required: false,
                        },
                    ]
                });
...
                    attributes: ["id_penginapan", "nama_penginapan", "sampul_penginapan"]
                });
...
                            attributes: ["id_paket_homestay", "nama_paket_homestay", "harga", "sampul_paket_homestay"],
                            required: false,
                        },
                    ]
                });
...
                const harga_satuan_sebelumnya = data_menu.harga * ExistDetailPesanan.jumlah;
...
                        id_detail_pesanan: items.id_detail_pesanan,
                        id_menu: items.id_menu,
                        nama_menu: items.tiket_detail_as ? "Tiket " + items.tiket_detail_as.nama_destinasi : items.menu_detail_as ? items.menu_detail_as.nama_menu : items.kamar_detail_as ? items.kamar_detail_as.nama_kamar : items.paket_wisata_detail_as ? items.paket_wisata_detail_as.nama_paket_wisata : items.homestay_detail_as 
                        ? items.homestay_detail_as.nama_paket_homestay 
                        : null, // untuk paket_homestay
                        jumlah: items.jumlah,
                        harga_satuan: items.harga_satuan,
                        sampul_menu: items.tiket_detail_as ? items.tiket_detail_as.sampul_destinasi : items.menu_detail_as ? items.menu_detail_as.sampul_menu : items.kamar_detail_as ? items.kamar_detail_as.sampul_kamar : items.paket_wisata_detail_as ? items.paket_wisata_detail_as.sampul_paket_wisata : items.homestay_detail_as 
                        ? items.homestay_detail_as.sampul_paket_homestay 
                        : null, // untuk paket_homestay
                    }))
                };
...
            return res.status(422).json({
                success: false,
                message: "Pesanan gagal dibatalkan karena status pembayaran tidak sesuai",
...
        console.log("Memproses Pembayaran ID:", id_pembayaran);

        // 1. Ambil Data Pembayaran & User
...
        if (['bayar', 'settlement', 'success'].includes(pembayaran.status_pembayaran)) {
...
        console.log("Snap Token Berhasil Dibuat:", snapToken);

        // 6. Update Database dengan Token Baru & Order ID Baru (Penting untuk Notifikasi nanti)
...
      const kode_pembayaran = `KPON-${Math.floor(100000 + Math.random() * 900000)}`;

      // === 5️⃣ Buat Data Pembayaran ===
...
        if (pembayaran.data_pembayaran_snap_token && pembayaran.data_pembayaran_snap_redirect_url) {
            return res.status(200).json({
                success: true,
...
            return res.status(422).json({ message: "Total pembayaran tidak boleh nol. Periksa pesanan Anda." });
        }
...
        const data_post_midtrans = await post_midtrans.json();
        if (post_midtrans.status !== 201) {
            return res.status(500).json({ message: "Gagal mendapatkan token pembayaran dari Midtrans", response: data_post_midtrans });
        }
...
            
            // Rumus Signature Midtrans: order_id + status_code + gross_amount + serverKey
...
const trxNotif = async (req, res) => {
    try {
        ...
        if (!data.order_id) {
            return res.status(400).json({ message: "Invalid Data: order_id is missing" });
        }
        ...
        if (!dataPembayaran) {
            ...
            return res.status(200).json({ message: 'Data pembayaran tidak ditemukan' });
        }
        ...
        if (dataPembayaran.status_pembayaran !== 'selesai' && dataPembayaran.status_pembayaran !== statusBaru) {
            
            const tgl_update = moment().tz("Asia/Jakarta").format("YYYY-MM-DD HH:mm:ss");

            // A. Update Tabel Pembayaran
            await dataPembayaran.update({
                status_pembayaran: statusBaru,
                tgl_pembayaran: statusBaru === 'bayar' ? tgl_update : dataPembayaran.tgl_pembayaran,
...
            detail_destinasi = await tbl_Wisata.findOne({
                where: { id_wisata: pesanan.id_destinasi },
                attributes: ["id_wisata", "id_admin", "id_admin_pengelola", "nama_destinasi"]
            });
...
            detail_destinasi = await tbl_Paket_wisata.findOne({
                where: { id_paket_wisata: pesanan.id_destinasi },
                attributes: ["id_paket_wisata", "id_admin", "id_admin_pengelola", "nama_paket_wisata"]
            });
...
            detail_destinasi = await tbl_Penginapan.findOne({
                where: { id_penginapan: pesanan.id_destinasi },
                attributes: ["id_penginapan", "id_admin", "id_admin_pengelola", "nama_penginapan"]
            });
...
            detail_destinasi = await tbl_Kuliner.findOne({
                where: { id_kuliner: pesanan.id_destinasi },
                attributes: ["id_kuliner", "id_admin", "id_admin_pengelola", "nama_kuliner"]
            });
...
            attributes: ["id_pembayaran", "id_wisatawan", "kode_pembayaran", "total_pembayaran", "tgl_pembayaran", "metode_pembayaran", "status_pembayaran"]
        });

        if (data_pembayaran.status_pembayaran === 'belum_bayar' || data_pembayaran.status_pembayaran === 'batal') {
            return res.status(422).json({
                status: 'error',
                success: false,
                message: data_pembayaran.status_pembayaran === 'belum_bayar' ? "Ticket belum dibayar!" : "Ticket dibatalkan!"
            });
        }
...
            tgl_pesanan: pesanan.createdAt,
            tgl_pesanan_selesai: pesanan.updatedAt,
            nama_destinasi: detail_destinasi.nama_destinasi || detail_destinasi.nama_paket_wisata || detail_destinasi.nama_penginapan || detail_destinasi.nama_kuliner,
            detail_pesanan: detail_pesanans.map(item => ({
                id_detail_pesanan: item.id_detail_pesanan,
                id_menu: item.id_menu,
                nama_menu: item.tiket_detail_as ? "Tiket " + item.tiket_detail_as.nama_destinasi : item.paket_wisata_detail_as ? item.paket_wisata_detail_as.nama_paket_wisata : item.kamar_detail_as ? item.kamar_detail_as.nama_kamar : item.homestay_detail_as ? item.homestay_detail_as.nama_paket_homestay : item.menu_detail_as ? item.menu_detail_as.nama_menu : "",
                jumlah: item.jumlah,
                harga_satuan: item.harga_satuan,
                sampul_menu: item.tiket_detail_as ? item.tiket_detail_as.sampul_destinasi : item.paket_wisata_detail_as ? item.paket_wisata_detail_as.sampul_paket_wisata : item.kamar_detail_as ? item.kamar_detail_as.sampul_kamar : item.homestay_detail_as ? item.homestay_detail_as.sampul_paket_homestay : item.menu_detail_as ? item.menu_detail_as.sampul_menu : ""
            }))
        };
...
        const data_destinasi = pesanan.nama_destinasi === 'tbl_destinasi' ? await tbl_Wisata.findOne({ where: { id_wisata: pesanan.id_destinasi } }) : pesanan.nama_destinasi === 'tbl_Paket_wisata' ? await tbl_Paket_wisata.findOne({ where: { id_paket_wisata: pesanan.id_destinasi } }) : pesanan.nama_destinasi === 'tbl_Penginapan' ? await tbl_Penginapan.findOne({ where: { id_penginapan: pesanan.id_destinasi } }) : await tbl_Kuliner.findOne({ where: { id_kuliner: pesanan.id_destinasi } });


        const detail_pesanan = await tbl_detail_pesanan.findOne({ where: { id_pesanan: data_pesanan.id_pesanan } });
        await data_pesanan.update({
...
        const nama_table = pesanan.nama_destinasi === 'tbl_destinasi' ? 'tbl_Wisata' : pesanan.nama_destinasi === 'tbl_Paket_wisata' ? 'tbl_Paket_wisata' : pesanan.nama_destinasi === 'tbl_Penginapan' ? 'tbl_Penginapan' : 'tbl_Kuliner';      
        const id_table = pesanan.nama_destinasi === 'tbl_destinasi' ? data_destinasi.id_wisata : pesanan.nama_destinasi === 'tbl_Paket_wisata' ? data_destinasi.id_paket_wisata : pesanan.nama_destinasi === 'tbl_Penginapan' ? data_destinasi.id_penginapan : data_destinasi.id_kuliner;
...
            });
        } else {
            const jumlah_pengunjung_aplikasi_update = data_pengunjung.jumlah_pengunjung_aplikasi + jumlah_pengunjung;
...
                        ? (data_destinasi.total_pengunjung_destinasi || 0) + jumlah_pengunjung
...
                        ? (data_destinasi.total_pengunjung_penginapan || 0) + jumlah_pengunjung
