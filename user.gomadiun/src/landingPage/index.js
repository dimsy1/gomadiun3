...
function Landing() {
  ...
  // --- USE EFFECT UNTUK PWA ---
  useEffect(() => {
    const handler = (e) => {
      // Mencegah Chrome 67+ otomatis menampilkan prompt default
      e.preventDefault();
      // Simpan event supaya bisa dipanggil nanti
      setDeferredPrompt(e);
      // Tampilkan tombol custom kita
  ...
  const getData = async () => {
    try {
      ...
      const response_wisata = await axios.get(
        `${process.env.REACT_APP_BACKEND_API_URL}/api/wisata/get_all`
      );
      if (response_wisata) {
        setWisataData(response_wisata.data.data);
      }
      const response_kuliner = await axios.get(
        `${process.env.REACT_APP_BACKEND_API_URL}/api/kuliner/get_all`
      );
      if (response_kuliner) {
        setKulinerData(response_kuliner.data.data);
      }
      const response_berita = await axios.get(
  ...
  const cetak = async (budget, jumlah) => {
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_BACKEND_API_URL}/api/paket_wisata/recomend`,
        {
          dana: budget,
          jumlah: jumlah,
        }
    ...
    <div>
      ...
      >
        ...
        {showInstallBtn && (
          ...
          >
            <i className="fa fa-download"></i> Install Aplikasi
          </button>
        )}
        ...
        <div className="content-header">
          ...
          <div className="text-content">
            <h1 className="hero-title">
              "Jelajahi wisata Madiun dan nikmati keindahannya"
            </h1>
            <p className="hero-subtitle">
              Temukan paket wisata sesuai budget yang Anda miliki
            </p>
          </div>
          ...
          <p className="hero-info">
            Masukkan budget dan klik tombol "Cari" untuk melihat rekomendasi
          </p>
        </div>

        <div
          className={`footer-header ${budget === "" ? "my-top-5" : ""}`}
        ></div>
      </div>
      {budget === "" ? (
        <>
          ...
          {TopDatas.length > 0 && <ContentTop dataTop={TopDatas} />}
          {DesaWisataDatas.length > 0 && (
            <ContentDesaWisata dataDesaWisata={DesaWisataDatas} />
          )}
          {WisataDatas.length > 0 && <ContentWisata dataWisata={WisataDatas} />}
          {BeritaDatas.length > 0 && <ContentBerita dataBerita={BeritaDatas} />}
        ...
        <div className="cover-recomend" id="paket-wisata-result">
          <div className="w-100">
            <div className="d-flex flex-row my-bottom-2">
              <span className="mx-1 text-bold text-size-14 mt-4">
                Hasil Pencarian berdasarkan budget anda
              </span>
            </div>
                ...
                {RecomendasiDatas.length === 0 ? (
                  ...
                  <div className="cover-recomended-item d-flex flex-wrap gap-4">
                    ...
                    {item.kategori_destinasi || "Paket Wisata"}
                            ...
                            <div className="rekomendasi-nama">
                          ...
                          <button
                            ...
                            onClick={() =>
                              Navigate(`paket_wisata/${item.id_paket_wisata}`)
                            }
                          >
                            Lihat Detail →
                          </button>
