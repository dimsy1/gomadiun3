...
const PaketWisata = ({ role, id_admin_login }) => {
    ...
    const getData = async (searchTerm = '') => {
        ...
        try {
            ...
            if (role === "admin" || role === "dinas") {
                url = `${process.env.REACT_APP_BACKEND_API_URL}/api/paket_wisata/get_data/byAdmin?keyword=${searchTerm}`;
            }
            else if (role === "admin pengelola") {
                url = `${process.env.REACT_APP_BACKEND_API_URL}/api/paket_wisata/get_data/byAdmin?byAdmin=${id_admin_login}&keyword=${searchTerm}`;
            }
            else if (role === "user pengelola") {
                url = `${process.env.REACT_APP_BACKEND_API_URL}/api/paket_wisata/get_data/byAdmin?byAdminPengelola=${id_admin_login}&keyword=${searchTerm}`;
            }
            else {
                url = `${process.env.REACT_APP_BACKEND_API_URL}/api/paket_wisata/get_data/byAdmin?keyword=${searchTerm}`;
            }
            ...
            const response = await axios.get(url);
            if (response) {
                setDataPaketWisata(response.data.data)
                setLoading(false);
            }
        } catch (error) {
            if (error.response.status === 401) {
    ...
    const currentPageData = DataPaketWisata.slice(offset, offset + itemsPerPage);
    const pageCount = Math.ceil(DataPaketWisata.length / itemsPerPage);
    ...
    const openModalEdit = (
        ...
        setFormDataEdit({
            ...
            sampul_paket_wisata: sampul,
            ruang_paket_wisata: ruang,
    ...
    const openModalFormVerifikasi = (id_paket_wisata, status, id_admin) => {
        if (role === "dinas") {
    ...
    const onCropComplete2 = async () => {
        try {
            const croppedImage2 = await getCroppedImg16_9_Berita(avatar2, croppedArea2);
    ...
    const handleSubmit = async (e) => {
        ...
        if (croppedImage && croppedImage2) {
            ...
            try {
                const response = await axios.post(`${process.env.REACT_APP_BACKEND_API_URL}/api/paket_wisata/add_data/byAdmin`, formDataObj)
                if (response) {
            ...
            if (!croppedImage && !croppedImage2) {
                setAlertaddImage('Mohon pilih foto sampul paket wisata dan ruang paket wisata')
                setTimeout(() => {
                    setAlertaddImage('');
                }, 3000)
            } else if (!croppedImage2) {
                setAlertaddImage('Mohon pilih foto ruang paket wisata')
                setTimeout(() => {
                    setAlertaddImage('');
                }, 3000)
            } else {
                setAlertaddImage('Mohon pilih foto sampul paket wisata')
                setTimeout(() => {
                    setAlertaddImage('');
                }, 3000)
            }
    ...
    const handleSubmitUpdate = async (e) => {
        ...
        formDataObj.append("nama_paket_wisata", formDataEdit.nama_paket_wisata);
        formDataObj.append("nib_paket_wisata", formDataEdit.nib_paket_wisata);
        formDataObj.append("kbli_paket_wisata", formDataEdit.kbli_paket_wisata);
        formDataObj.append("alamat_paket_wisata", formDataEdit.alamat_paket_wisata);
        formDataObj.append("status_paket_wisata", formDataEdit.status_paket_wisata);
        formDataObj.append("npwp_paket_wisata", formDataEdit.npwp_paket_wisata);
        formDataObj.append("npwp_pemilik_paket_wisata", formDataEdit.npwp_pemilik_paket_wisata);
        formDataObj.append("desk_paket_wisata", formDataEdit.desk_paket_wisata);
        formDataObj.append("content_paket_wisata", formDataEdit.content_paket_wisata);
        formDataObj.append("harga_paket_wisata", formDataEdit.harga_paket_wisata);
        formDataObj.append("status_jalan", formDataEdit.status_jalan);
        formDataObj.append("jenis_kendaraan", formDataEdit.jenis_kendaraan);
        formDataObj.append("kontak_person_paket_wisata", formDataEdit.kontak_person_paket_wisata);
        ...
        try {
            const response = await axios.put(`${process.env.REACT_APP_BACKEND_API_URL}/api/paket_wisata/update/byAdmin/${formDataEdit.id_paket_wisata}`, formDataObj)
            if (response) {
                setResponseMessage(response.data.message);
                setResponseMessageStatus(response.data.status);
                closeModalEdit();
    ...
    const handleSubmitDelete = async (e) => {
        ...
        try {
            const response = await axios.delete(`${process.env.REACT_APP_BACKEND_API_URL}/api/paket_wisata/delete/byAdmin/${DataDelete.id}`)
            if (response) {
                setResponseMessage(response.data.message);
                setResponseMessageStatus(response.data.status);
                closeModalDelete();
                                    ...
                                    <form onSubmit={handleSubmit} className="modal-form">
                                        ...
                                        {(formData.id_admin && formData.id_admin_pengelola) && (
                                            <div className="form-group">
                                                ...
                                                <input
                                                    type="text"
                                                    name="npwp_pemilik_paket_wisata"
                                                    value={formData.npwp_pemilik_paket_wisata}
                                                    onChange={handleInputChange}
                                                />
                                            </div>
                                        )}
                                        <div className="form-group">
                                            ...
                                            <input
                                                ...
                                                value={formData.nama_paket_wisata}
                                                onChange={handleInputChange}
                                                required
                                            />
                                        </div>
                                        <div className="form-group">
                                            <label htmlFor="name">NIB Paket Wisata</label>
                                            <input
                                                type="text"
                                                name="nib_paket_wisata"
                                                value={formData.nib_paket_wisata}
                                                onChange={handleInputChange}
                                            />
                                        </div>
                                        <div className="form-group">
                                            <label htmlFor="name">KBLI Paket Wisata</label>
                                            <input
                                                type="text"
                                                name="kbli_paket_wisata"
                                                value={formData.kbli_paket_wisata}
                                                onChange={handleInputChange}
                                            />
                                        </div>
                                        <div className="form-group">
                                            <label htmlFor="name">Alamat Paket Wisata</label>
                                            <input
                                                type="text"
                                                name="alamat_paket_wisata"
                                                value={formData.alamat_paket_wisata}
                                                onChange={handleInputChange}
                                                required
                                            />
                                        </div>
                                        <div className="form-group">
                                            <label htmlFor="status_paket_wisata">Status Paket Wisata</label>
                                            <select
                                                className="form-control"
                                                name="status_paket_wisata"
                                                style={{ width: '100%' }}
                                                value={formData.status_paket_wisata}
                                                onChange={handleInputChange}
                                                required
                                            >
                                        ...
                                        <div className="form-group">
                                            <label htmlFor="name">NPWP Paket Wisata</label>
                                            <input
                                                type="text"
                                                name="npwp_paket_wisata"
                                                value={formData.npwp_paket_wisata}
                                                onChange={handleInputChange}
                                            />
                                        </div>
                                        <div className="form-group">
                                            <label htmlFor="desk_paket_wisata">Deskripsi Paket Wisata</label>
                                            <textarea
                                                name="desk_paket_wisata"
                                                value={formData.desk_paket_wisata}
                                                onChange={handleInputChange}
                                                rows="5"
                                                style={{ width: '100%' }}
                                                required
                                        ...
                                        <div className="form-group">
                                            <label htmlFor="content_paket_wisata">Content Paket Wisata</label>
                                              <ReactQuill
                                                theme="snow"
                                                value={formData.content_paket_wisata}
                                                onChange={(value) => setFormData({ ...formData, content_paket_wisata: value })}
                                              />
                                            </div>
                                        <div className="form-group">
                                            <label htmlFor="harga_paket_wisata">Harga Tiket Paket Wisata</label>
                                            <input
                                                type="text"
                                                name="harga_paket_wisata"
                                                value={formData.harga_paket_wisata}
                                                onChange={handleInputChange}
                                                required
                                            />
                                        </div>
                                        <div className="form-group">
                                            <label htmlFor="status_jalan">Kondisi Jalan Paket Wisata</label>
                                            <select
                                                className="form-control"
                                                name="status_jalan"
                                                style={{ width: '100%' }}
                                        ...
                                        <div className="form-group">
                                            <label htmlFor="kontak_person_paket_wisata">No. telp Paket Wisata</label>
                                            <input
                                                type="text"
                                                name="kontak_person_paket_wisata"
                                                value={formData.kontak_person_paket_wisata}
                                                onChange={handleInputChange}
                                                required
                                            />
                                        </div>
                                    ...
                                    <form className="modal-form" onSubmit={handleSubmitUpdate}>
                                        ...
                                        <div className="form-group">
                                            ...
                                            <input
                                                type="text"
                                                name="npwp_pemilik_paket_wisata"
                                                value={formDataEdit.npwp_pemilik_paket_wisata}
                                        ...
                                        <div className="form-group">
                                            ...
                                            <input
                                                type="text"
                                                name="nama_paket_wisata"
                                                value={formDataEdit.nama_paket_wisata}
                                                onChange={handleInputChangeEdit}
                                                required
                                            />
                                        </div>
                                        <div className="form-group">
                                            ...
                                            <input
                                                type="text"
                                                name="nib_paket_wisata"
                                                value={formDataEdit.nib_paket_wisata}
                                                onChange={handleInputChangeEdit}
                                            />
                                        </div>
                                        <div className="form-group">
                                            ...
                                            <input
                                                type="text"
                                                name="kbli_paket_wisata"
                                                value={formDataEdit.kbli_paket_wisata}
                                                onChange={handleInputChangeEdit}
                                            />
                                        </div>
                                        <div className="form-group">
                                            ...
                                            <input
                                                type="text"
                                                name="alamat_paket_wisata"
                                                value={formDataEdit.alamat_paket_wisata}
                                                onChange={handleInputChangeEdit}
                                                required
                                            />
                                        </div>
                                        <div className="form-group">
                                            ...
                                            <select
                                                ...
                                                name="status_paket_wisata"
                                                style={{ width: '100%' }}
                                                value={formDataEdit.status_paket_wisata}
                                                onChange={handleInputChangeEdit}
                                                required
                                            >
                                                <option value="">--- Pilih Status Kempemilikan ---</option>
                                        ...
                                        <div className="form-group">
                                            ...
                                            <input
                                                type="text"
                                                name="npwp_paket_wisata"
                                                value={formDataEdit.npwp_paket_wisata}
                                                onChange={handleInputChangeEdit}
                                            />
                                        </div>
                                        <div className="form-group">
                                            ...
                                            <textarea
                                                name="desk_paket_wisata"
                                                value={formDataEdit.desk_paket_wisata}
                                                onChange={handleInputChangeEdit}
                                                rows="5"
                                                style={{ width: '100%' }}
                                                required
                                        ...
                                        <div className="form-group">
                                            ...
                                            <ReactQuill
                                                theme="snow"
                                                value={formDataEdit.content_paket_wisata}
                                                onChange={(value) =>
                                                setFormDataEdit({ ...formDataEdit, content_paket_wisata: value })
                                                }
                                            />
                                            </div>
                                        <div className="form-group">
                                            ...
                                            <input
                                                type="text"
                                                name="harga_paket_wisata"
                                                value={formDataEdit.harga_paket_wisata}
                                                onChange={handleInputChangeEdit}
                                                required
                                            />
                                        </div>
                                        ...
                                        <div className="form-group">
                                            ...
                                            <input
                                                type="text"
                                                name="kontak_person_paket_wisata"
                                                value={formDataEdit.kontak_person_paket_wisata}
                                                onChange={handleInputChangeEdit}
                                                required
                                            />
                                        </div>
                                        <div className="form-group">
                                            ...
                                            <div>
                                                <img src={`${formDataEdit.sampul_paket_wisata}`} alt={formDataEdit.sampul_paket_wisata} style={{ width: '150px' }} />
                                            </div>
                                        ...
                                        <div className="form-group">
                                            ...
                                            <div>
                                                {formDataEdit.ruang_paket_wisata ? (
                                                    <img src={`${formDataEdit.ruang_paket_wisata}`} alt={formDataEdit.ruang_paket_wisata} style={{ width: '150px' }} />
                                                ) : (
                    ...
                    <div className="col-lg-12 grid-margin stretch-card mt-3">
                        <div className="card">
                            <div className="card-body">
                                <h4 className="card-title">Tabel Data Paket Wisata</h4>
                                {responseMessage && (
                                ...
                                <div className="table-responsive">
                                    <table className="table table-hover">
                                        <thead>
                                            <tr>
                                                <th>#</th>
                                                <th>Destinasi Paket Wisata</th>
                                                <th>Desa Wisata</th>
                                                <th>Pemilik Paket Wisata</th>
                                                <th>Pengelola</th>
                                                <th>NPWP</th>
                                                ...
                                                <>
                                                    {currentPageData.map((item, index) => (
                                                        <tr key={index}>
                                                            ...
                                                            <td>Milik {item.status_paket_wisata}</td>
                                                            ...
                                                            <td className={item.npwp_paket_wisata ? "text-dark" : "text-danger"}>{item.npwp_paket_wisata ? item.npwp_paket_wisata : "Undifined"}</td>
                                                            {item.detail_admin_verified ? (
                                                                <td><label className={`badge ${item.status_verifikasi === "verified" ? " badge-success" : " badge-danger"}`}
                                                                    ...
                                                                    :   () => openModalFormVerifikasi(item.id_paket_wisata, item.status_verifikasi, item.detail_admin_verified.id_admin)}
                                                                >
                                                                ...
                                                                <td><label className={`badge badge-warning`}
                                                                    onClick={() => openModalFormVerifikasi(item.id_paket_wisata, item.status_verifikasi, item.detail_admin_verified ? item.detail_admin_verified.id_admin : 2)}
                                                                >Belum diverifikasi</label></td>
                                                            )}
                                                            <td><Link to={`/paketwisata/${item.id_paket_wisata}`}>Lihat detail</Link>
                                                                <FontAwesomeIcon className='mx-2' icon={faEdit} onClick={() => openModalEdit(
                                                                    item.detail_admin.id_admin,
                                                                    item.id_paket_wisata,
                                                                    ...
                                                                    item.nama_paket_wisata ? item.nama_paket_wisata : '',
                                                                    item.nib_paket_wisata ? item.nib_paket_wisata : '',
                                                                    item.kbli_paket_wisata ? item.kbli_paket_wisata : '',
                                                                    item.alamat_paket_wisata ? item.alamat_paket_wisata : '',
                                                                    item.status_paket_wisata ? item.status_paket_wisata : '',
                                                                    item.npwp_pemilik_paket_wisata ? item.npwp_pemilik_paket_wisata : '',
                                                                    item.npwp_paket_wisata ? item.npwp_paket_wisata : '',
                                                                    item.desk_paket_wisata ? item.desk_paket_wisata : '',
                                                                    item.content_paket_wisata ? item.content_paket_wisata : '',
                                                                    item.harga_paket_wisata ? item.harga_paket_wisata : '',
                                                                    item.status_jalan ? item.status_jalan : '', 
                                                                    item.jenis_kendaraan ? item.jenis_kendaraan : '', 
                                                                    item.kontak_person_paket_wisata ? item.kontak_person_paket_wisata : '',
                                                                    item.sampul_paket_wisata,
                                                                    item.ruang_paket_wisata,
                                                                )} />
                                                                <FontAwesomeIcon icon={faTrash} onClick={() => openModalDelete(item.id_paket_wisata, item.nama_paket_wisata, item.sampul_paket_wisata)} />
                                                            </td>
                                                        </tr>
                                                    ))}
                                                </>
