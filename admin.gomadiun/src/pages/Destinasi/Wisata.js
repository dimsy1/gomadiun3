...
const TableWisata = ({ role, id_admin_login }) => {
    ...
    const [DataKecamatanOption, setDataKecamatanOption] = useState([]);
...
L.Icon.Default.mergeOptions({
  ...
  shadowUrl: 'https://cdnjs.cloudflare.com/ajax/libs/leaflet/1.7.1/images/marker-shadow.png',
    ...
    const handleMapClick = (latlng) => {
        setFormData({ ...formData, latitude: latlng.lat.toFixed(6), longitude: latlng.lng.toFixed(6) });
      };

    const handleMapClickEdit = (latlng) => {
        setFormDataEdit({ ...formDataEdit, latitude: latlng.lat.toFixed(6), longitude: latlng.lng.toFixed(6) });
      };
    ...
    const getDataKecamatanOption = async (id) => {
        setDataKecamatanOption([])
        try {
            const url = `${process.env.REACT_APP_BACKEND_API_URL}/api/kecamatan/get_all/`;

            const response = await axios.get(url);
            if (response) {
                setDataKecamatanOption(response.data.data)
            }
        } catch (error) {
            if (error.response.status === 401) {
                navigate('/');
    ...
    useEffect(() => {
        if (showModal) {
            const initSelect2 = (selector, fieldName, namePlaceholder) => {
                $(selector).select2({
                    ...
                    if (fieldName === 'id_admin') {
                        ...
                        getDataKecamatanOption(selectedValue);
                    }
                });
            };
            ...
            initSelect2('#id_kecamatan', 'id_kecamatan', 'Kecamatan');
    ...
    useEffect(() => {
        if (showModalEdit) {
            ...
            initSelect2('#id_kecamatan_edit', 'id_kecamatan', 'Kecamatan');

            return () => {
                $('#id_admin_pengelola_edit').select2('destroy');
                $('#id_desaWisata_edit').select2('destroy');
                $('#id_kecamatan_edit').select2('destroy');
            };
    ...
    const openModal = () => {
        if (role === "admin pengelola") {
            ...
            getDataKecamatanOption(id_admin_login);
        }
        setShowModal(true);
        setIsClosing(false);
    };
    ...
    const openModalEdit = (
        ...
        if (role === "admin" || role === "dinas" || role === "admin pengelola") {
            ...
            getDataKecamatanOption(id_admin);
        }
        setFormDataEdit({
            id_admin: id_admin,
   ...
   const openModalFormVerifikasi = (id_wisata, status, admin) => {
    if (role === "dinas") {
   ...
   const handleInputChangeFormVerifikasi = (e) => {
    ...
    const handleSubmitUpdate = async (e) => {
        ...
        formDataObj.append("id_kecamatan", formDataEdit.id_kecamatan);
        ...
        formDataObj.append("kategori", formDataEdit.kategori);
...
console.log('ID Wisata:', DataFormVerifikasi.id_wisata);
console.log('ID Admin:', DataFormVerifikasi.id_admin);
console.log('Status Verifikasi:', DataFormVerifikasi.status_verifikasi);
    ...
    return (
                                    ...
                                    <form onSubmit={handleSubmit} className="modal-form">
                                        ...
                                        <div className="form-group">
                                            ...
                                            >
                                                ...
                                                {formData.id_admin && (
                                                    <>
                                                        {DataKecamatanOption.map((item, index) => (
                                        ...
                                        <div className="form-group">
                                <label htmlFor="kategori">Kategori Wisata</label>
                                <select
                                    className="form-control"
                                    name="kategori"
                                    style={{ width: '100%' }}
      ...
      <MapContainer center={[-7.5, 112.7]} zoom={8} style={{ height: '400px', marginTop: '20px' }}>
        <TileLayer
                                        ...
                                        {(role === "admin" || role === "dinas" || role === "admin pengelola") && (
                                            <>
                                                ...
                                                <div className="form-group">
                                                    ...
                                                    <select
                                                        className="form-control"
                                                        id="id_kecamatan_edit"
                                                        name="id_kecamatan"
                                                        style={{ width: '100%' }}
                                                        value={formDataEdit.id_kecamatan}
                                                        required
                                                    >
                                                        <option value="">select</option>
                                    ...
                                    <div className="form-group">
                                        ...
                                        {/* <h3>Pilih Lokasi di Peta:</h3>
                                            <MapContainer center={[-7.5, 112.7]} zoom={8} style={{ height: '400px', marginTop: '20px' }}>
                                                ...
                                                url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
                                                />
                                                <LocationMarker onSelect={handleMapClickEdit} />
                                                {formDataEdit.latitude && formDataEdit.longitude && (
                                                <Marker position={[formDataEdit.latitude, formDataEdit.longitude]} />
                                                )}
                                            </MapContainer> */}
                                </div>
                                <div className="modal-footer">
  ...
  <label className="badge badge-warning"
    onClick={() => openModalFormVerifikasi(item.id_wisata, item.status_verifikasi, item.detail_admin_verified)}>
    Belum diverifikasi
  </label>
</td>
                                                            )}
                                                            <td><Link to={`/wisata/${item.id_wisata}`}>Lihat detail</Link>
                                                                <FontAwesomeIcon className='mx-2' icon={faEdit} onClick={() => openModalEdit(
                                                                    ...
                                                                    item.detail_kecamatan ? item.detail_kecamatan.id_kecamatan : '',
