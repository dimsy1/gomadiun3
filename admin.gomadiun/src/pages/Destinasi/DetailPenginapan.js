...
const DetailPenginapan = ({ role, id_admin_login }) => {
    ...
    const [activeTab, setActiveTab] = useState('Detail Penginapan');
    ...
    const getData = async (tahun = '') => {
        ...
        setDataUsers([]);
        setDataDetailPenginapan([]);
        setDataAjuan([]);
        try {
            if (role === "admin") {
                getDataOption()
            }
            const url = `${process.env.REACT_APP_BACKEND_API_URL}/api/penginapan/detail/byAdmin/${id_penginapan}`;

            const response = await axios.get(url);
            if (response) {
                setDataDetailPenginapan(response.data.data)
                setLoadingMuatHalDetail(false);
                if (response.data.data[0].detail_fasilitas_penginapan.length !== 0) {
                    setListFasilitas(
                        response.data.data[0].detail_fasilitas_penginapan.map((item) => item.value_fasilitas_penginapan)
                    );                
                }

                try {
                    const url = `${process.env.REACT_APP_BACKEND_API_URL}/api/pengunjung/get_all/byAdmin/${id_penginapan}?keywoard=tbl_penginapan&tahun=${tahun}`;
                    ...
                    if (response) {
                        ...
                        try {
                            const url = `${process.env.REACT_APP_BACKEND_API_URL}/api/pengunjung/get_all_noverified/byAdmin/${id_penginapan}?keywoard=tbl_penginapan`;
    ...
    const exportDataPengunjung = async () => {
        try {
            const response = await axios.get(`http://localhost:3001/api/pengunjung/export-data-pengunjung/penginapan/${id_penginapan}`, { responseType: 'json' });
            ...
            const fileName = `data_pengunjung_penginapan_${destinationName.replace(/\s+/g, '_')}_${Date.now()}.xlsx`;
    ...
    const openModalChangesFasilitas = (id_penginapan, status) => {
        if (status === "update") {
    ...
    const openModalFormVerifikasi = (id_penginapan, status) => {
        if (role === "dinas") {
    ...
    const handleSubmitUpdateFasilitas = async (e) => {
        ...
        try {
            ...
            const response = await axios.post(`${process.env.REACT_APP_BACKEND_API_URL}/api/penginapan/add_fasilitas/byAdmin`, {
                id_penginapan: formDataEditFasilitas.id,
            ...
            <div className="content-wrapper">
                <div className="row">
                    <div className='d-flex w-100'>
                        <h4 className="card-title">Detail Penginapan</h4>
                    </div>
                            ...
                            <div className="card-body">
                                ...
                                <nav className="navbar navbar-expand-custom navbar-mainbg">
                                    <div className="collapse navbar-collapse" id="navbarSupportedContent">
                                        <ul className="navbar-nav">
                                            ...
                                            {['Detail Penginapan', 'Data Pengunjung'].map((tab) => (
                                ...
                                {!loadingMuatHalDetail ? (
                                    ...
                                    <div className='d-flex flex-column my-5'>
                                        {DataDetailPenginapan[0] ? (
                                            <div>
                                                {DataDetailPenginapan.map((item, index) => (
                                                    <div className='cover-detail-content' style={{ display: activeTab === "Detail Penginapan" ? "" : "none" }}>
                                                        <div className='detail-content'>
                                                            <div className='detail-content-row'>
                                                                <span className='title-name'>Nama Penginapan</span>
                                                                <span>:</span>
                                                                <span className='subtitle-name'>{item.nama_penginapan}</span>
                                                            </div>
                                                            <div className='detail-content-row'>
                                                            ...
                                                            <div className='detail-content-row'>
                                                                <span className='title-name'>Admin Penginapan</span>
                                                                <span>:</span>
                                                                <span className='subtitle-name'>{item.detail_admin.nama_admin}</span>
                                                            </div>
                                                            <div className='detail-content-row'>
                                                                <span className='title-name'>User Penginapan</span>
                                                                <span>:</span>
                                                            ...
                                                            <div className='detail-content-row'>
                                                                ...
                                                                <span>:</span>
                                                                {item.npwp_pemilik_penginapan ? (
                                                                    <span className='subtitle-name'>{item.npwp_pemilik_penginapan}</span>
                                                                ) : (
                                                                    <span className='subtitle-name text-danger'>Undifined</span>
                                                                )}
                                                            </div>
                                                                ...
                                                                <span className='subtitle-name'>
                                                                    {item.detail_admin_verified ? (
                                                                        ...
                                                                        <label className={`badge badge-warning mt-0`}
                                                                            onClick={() => openModalFormVerifikasi(item.id_penginapan, item.status_verifikasi)}
                                                                        >Belum diverifikasi</label>
                                                                    )}
                                                                </span>
                                                            </div>
                                                            <div className='detail-content-row'>
                                                                <span className='title-name'>KBLI Penginapan</span>
                                                                <span>:</span>
                                                                {item.kbli_penginapan ? (
                                                                    <span className='subtitle-name'>{item.kbli_penginapan}</span>
                                                                ) : (
                                                                    <span className='subtitle-name text-danger'>Undifined</span>
                                                                )}
                                                            </div>
                                                            <div className='detail-content-row'>
                                                                <span className='title-name'>NIB Penginapan</span>
                                                                <span>:</span>
                                                                {item.nib_penginapan ? (
                                                                    <span className='subtitle-name'>{item.nib_penginapan}</span>
                                                                ) : (
                                                                    <span className='subtitle-name text-danger'>Undifined</span>
                                                                )}
                                                            </div>
                                                            <div className='detail-content-row'>
                                                                <span className='title-name'>Alamat Penginapan</span>
                                                                <span>:</span>
                                                                {item.alamat_penginapan ? (
                                                                    <span className='subtitle-name'>{item.alamat_penginapan}</span>
                                                                ) : (
                                                                    <span className='subtitle-name text-danger'>Undifined</span>
                                                                )}
                                                            </div>
                                                            <div className='detail-content-row'>
                                                                <span className='title-name'>Status Penginapan</span>
                                                                <span>:</span>
                                                                {item.status_penginapan ? (
                                                                    <span className='subtitle-name'>Milik {item.status_penginapan}</span>
                                                                ) : (
                                                                    <span className='subtitle-name text-danger'>Undifined</span>
                                                                )}
                                                            </div>
                                                            <div className='detail-content-row'>
                                                                <span className='title-name'>NPWP Penginapan</span>
                                                                <span>:</span>
                                                                {item.npwp_penginapan ? (
                                                                    <span className='subtitle-name'>{item.npwp_penginapan}</span>
                                                                ) : (
                                                                    <span className='subtitle-name text-danger'>Undifined</span>
                                                                )}
                                                            </div>
                                                            <div className='detail-content-row'>
                                                                <span className='title-name'>Harga Kamar Terendah</span>
                                                                <span>:</span>
                                                                {item.harga_terendah_penginapan ? (
                                                                    <span className='subtitle-name'>{item.harga_terendah_penginapan}</span>
                                                                ) : (
                                                                    <span className='subtitle-name text-danger'>Undifined</span>
                                                                )}
                                                            </div>
                                                            <div className='detail-content-row'>
                                                                <span className='title-name'>Kontak Penginapan</span>
                                                                <span>:</span>
                                                                <span className='subtitle-name'>{item.kontak_person_penginapan}</span>
                                                            </div>
                                                            <div className='detail-content-row'>
                                                                <span className='title-name'>Total Pengunjung Penginapan</span>
                                                                <span>:</span>
                                                                <span className='subtitle-name'>{item.total_pengunjung_penginapan} Pengunjung</span>
                                                            </div>
                                                            <div className='detail-content-row'>
                                                            <span className='title-name'>Kategori Penginapan</span>
                                                            <span>:</span>
                                                            <span className='subtitle-name'>
                                                                    {item.kategori_penginapan === "Hotel" ? "Hotel" : item.kategori_penginapan === "Homestay" ? "Homestay" : "Kategori tidak diketahui"}
                                                                </span>
                                                                </div>
                                                                <div className='detail-content-row'>
                                                                <span className='title-name'>Kelas Penginapan</span>
                                                                <span>:</span>
                                                                <span className='subtitle-name'>{item.kelas_penginapan} Bintang</span>
                                                            </div>
                                                            <div className='detail-content-row'>
                                                                <span className='title-name'>Fasilitas Penginapan</span>
                                                                <span>:</span>
                                                                <div className='subtitle-name'>
                                                                    {item.detail_fasilitas_penginapan.length !== 0 ? (
                                                                        <>
                                                                            <div className='d-flex flex-column mb-2'>
                                                                                {item.detail_fasilitas_penginapan.map((item, index) => {
                                                                                    return (
                                                                                        <span key={index} className='px-1'>- {item.fasilitas}</span>
                                                                                    )
                                                                                })}
                                                                            </div>
                                                                            <label className={`badge badge-success mt-0`}
                                                                                onClick={() => openModalChangesFasilitas(item.id_penginapan, "update")}
                                                                            >
                                                                                Ubah list fasilitas
                                                                            </label>
                                                                        </>
                                                                        ...
                                                                        <label className={`badge badge-warning mt-0`}
                                                                            onClick={() => openModalChangesFasilitas(item.id_penginapan, "add")}
                                                                        >Tambahkan list fasilitas</label>
                                                                    )}
                                                                </div>
                                                            </div>
                                                            <div className='detail-content-row'>
                                                                <span className='title-name'>Deskripsi Penginapan</span>
                                                                <span>:</span>
                                                                <span className='subtitle-name' style={{ whiteSpace: 'pre-wrap', width: 850 }}>{item.desk_penginapan}</span>
                                                            </div>

                                                            <div className='detail-content-row-wisata'>
                                                                <span className='title-name'>Foto Sampul Penginapan</span>
                                                                <span>:</span>
                                                                <img src={item.sampul_penginapan} alt={item.sampul_penginapan} />
                                                            </div>

                                                            <div className='detail-content-row-wisata'>
                                                                <span className='title-name'>Foto Ruang Penginapan</span>
                                                                <span>:</span>
                                                                {item.ruang_penginapan ? (
                                                                    <img src={item.ruang_penginapan} alt={item.ruang_penginapan} />
                                                                ) : (
                                                                    <span className='subtitle-name text-danger'>Undifined</span>
                                                                )}
                                                            </div>
                                                            ...
                                                            <div className='detail-content-row-wisata'>
                                                                ...
                                                                {item.ruang_penginapan_dua ? (
                                                                    <img src={item.ruang_penginapan_dua} alt={item.ruang_penginapan_dua} />
                                                                ) : (
                                                                    <span className='subtitle-name text-danger'>Undifined</span>
                                                                )}
                                                            </div>
                                                            ...
                                                            <div className='detail-content-row-wisata'>
                                                                ...
                                                                {item.ruang_penginapan_tiga ? (
                                                                    <img src={item.ruang_penginapan_tiga} alt={item.ruang_penginapan_tiga} />
                                                                ) : (
                                                                    <span className='subtitle-name text-danger'>Undifined</span>
                                                                )}
                                                            </div>
                                                            ...
                                                            <div className='detail-content-row-wisata'>
                                                                ...
                                                                {item.ruang_penginapan_empat ? (
                                                                    <img src={item.ruang_penginapan_empat} alt={item.ruang_penginapan_empat} />
                                                                ) : (
                                                                    <span className='subtitle-name text-danger'>Undifined</span>
                                                                )}
                                                            </div>
                                                            ...
                                                            <div className='detail-content-row-wisata'>
                                                                ...
                                                                {item.ruang_penginapan_lima ? (
                                                                    <img src={item.ruang_penginapan_lima} alt={item.ruang_penginapan_lima} />
                                                                ) : (
                                                                    <span className='subtitle-name text-danger'>Undifined</span>
                                                                )}
                                                            </div>
                                                            ...
                                                            <div className='detail-content-row'>
                                                                <span className='title-name'>Latitude</span>
                                                                <span>:</span>
                                                                <span className='subtitle-name'>{item.latitude}</span>
                                                            </div>
                                                            <div className='detail-content-row'>
                                                                <span className='title-name'>Longitude</span>
                                                                <span>:</span>
                                                                <span className='subtitle-name'>{item.longitude}</span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                ))}
                                                ...
                                                <div className='mt-5' style={{ display: activeTab === "Data Pengunjung" ? "" : "none" }}>
                                                    ...
                                                    {DataAjuan.length !== 0 && (
                                                        <>
                                                            <h6 className="card-title">Tabel Data Ajuan Pengunjung {DataDetailPenginapan[0]?.nama_penginapan}</h6>
                                                    ...
                                                    <h4 className="card-title">Tabel Data Pengunjung {DataDetailPenginapan[0]?.nama_desaWisata}</h4>
                                                    <div className="container-top">
                                                        ...
                                                        <button 
                                                                    ...
                                                                    onClick={() => exportDataPengunjung(id_penginapan, 'penginapan')} // Menyertakan idWisata dan type 'wisata' saat mengklik tombol
