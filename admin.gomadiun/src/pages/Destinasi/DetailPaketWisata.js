...
const DetailPaketWisata = ({ role, id_admin_login }) => {
    const { id_paket_wisata } = useParams();
    const navigate = useNavigate();
    const [DataUsers, setDataUsers] = useState([]);
    const [DataDetailPaketWisata, setDataDetailPaketWisata] = useState([]);
    ...
    const [activeTab, setActiveTab] = useState('Detail Paket Wisata');
    ...
    const getData = async (tahun = '') => {
        ...
        setDataUsers([]);
        setDataDetailPaketWisata([]);
        setDataAjuan([]);
        try {
            if (role === "admin") {
                getDataOption()
            }
            const url = `${process.env.REACT_APP_BACKEND_API_URL}/api/paket_wisata/detail/byAdmin/${id_paket_wisata}`;

            const response = await axios.get(url);
            if (response) {
                setDataDetailPaketWisata(response.data.data)
                setLoadingMuatHalDetail(false);
                if (response.data.data[0].detail_fasilitas_paket_wisata.length !== 0) {
                    setListFasilitas(
                        response.data.data[0].detail_fasilitas_paket_wisata.map((item) => item.value_fasilitas_paket_wisata)
                    );                
                }

                try {
                    const url = `${process.env.REACT_APP_BACKEND_API_URL}/api/pengunjung/get_all/byAdmin/${id_paket_wisata}?keywoard=tbl_paket_wisata&tahun=${tahun}`;
                    ...
                    if (response) {
                        ...
                        try {
                            const url = `${process.env.REACT_APP_BACKEND_API_URL}/api/pengunjung/get_all_noverified/byAdmin/${id_paket_wisata}?keywoard=tbl_paket_wisata`;
    ...
    const exportDataPengunjung = async () => {
        try {
            const response = await axios.get(`http://localhost:3001/api/pengunjung/export-data-pengunjung/paket_wisata/${id_paket_wisata}`, { responseType: 'json' });
    
            // Ambil nama destinasi dari respons
            const destinationName = response.data.destinationName || 'Unknown';
            console.log("Nama Destinasi: ", destinationName);
    
            // Membuat nama file dengan nama destinasi
            const fileName = `data_pengunjung_paket_wisata_${destinationName.replace(/\s+/g, '_')}_${Date.now()}.xlsx`;
    
            // Konversi base64 ke Uint8Array
            const byteCharacters = atob(response.data.file);  // Decode base64
            const byteArrays = [];
    
            // Konversi base64 string ke dalam byte array
    ...
    const openModalChangesMaps = (id_paket_wisata, status, url) => {
        if (status === "update") {
            setFormDataEditMaps({
                id: id_paket_wisata,
                statusModal: status,
    ...
    const openModalChangesFasilitas = (id_paket_wisata, status) => {
        if (status === "update") {
    ...
    const openModalFormVerifikasi = (id_paket_wisata, status) => {
        if (role === "dinas") {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post(`${process.env.REACT_APP_BACKEND_API_URL}/api/pengunjung/add_data/byAdmin?keywoard=tbl_paket_wisata`, {
    ...
    const handleSubmitUpdateMaps = async (e) => {
        ...
        try {
            const response = await axios.put(`${process.env.REACT_APP_BACKEND_API_URL}/api/maps_paket_wisata/update/byAdmin/${formDataEditMaps.id}`, {
    ...
    const handleSubmitUpdateVerifikasi = async (e) => {
        ...
        try {
            const response = await axios.put(`${process.env.REACT_APP_BACKEND_API_URL}/api/paket_wisata/verif/byAdmin/${DataFormVerifikasi.id_paket_wisata}`, {
    ...
    const handleSubmitUpdateFasilitas = async (e) => {
        ...
        try {
            ...
            const response = await axios.post(`${process.env.REACT_APP_BACKEND_API_URL}/api/paket_wisata/add_fasilitas/byAdmin`, {
                id_paket_wisata: formDataEditFasilitas.id,
            ...
            <div className="content-wrapper">
                <div className="row">
                    <div className='d-flex w-100'>
                        <h4 className="card-title">Detail  Paket Wisata</h4>
                    </div>
                            ...
                            <div className="modal-content slideDown" onClick={e => e.stopPropagation()}>
                                ...
                                <div className="modal-body">
                                    <form className="modal-form" onSubmit={handleSubmitUpdateMaps}>
                                        <div className="form-group">
                                            <label htmlFor="url">Link url Maps  Paket Wisata</label>
                                            <input
                                                type="text"
                                                name="url"
                            ...
                            <div className="card-body">
                                ...
                                <nav className="navbar navbar-expand-custom navbar-mainbg">
                                    <div className="collapse navbar-collapse" id="navbarSupportedContent">
                                        <ul className="navbar-nav">
                                            ...
                                            {['Detail Paket Wisata', 'Data Pengunjung'].map((tab) => (
                                ...
                                {!loadingMuatHalDetail ? (
                                    ...
                                    <div className='d-flex flex-column my-5'>
                                        {DataDetailPaketWisata[0] ? (
                                            <div>
                                                {DataDetailPaketWisata.map((item, index) => (
                                                    <div className='cover-detail-content' style={{ display: activeTab === "Detail Paket Wisata" ? "" : "none" }}>
                                                        <div className='detail-content'>
                                                            <div className='detail-content-row'>
                                                                <span className='title-name'>Nama Paket Wisata</span>
                                                                <span>:</span>
                                                                <span className='subtitle-name'>{item.nama_paket_wisata}</span>
                                                            </div>
                                                            <div className='detail-content-row'>
                                                            ...
                                                            <div className='detail-content-row'>
                                                                <span className='title-name'>Admin  Paket Wisata</span>
                                                                <span>:</span>
                                                                <span className='subtitle-name'>{item.detail_admin.nama_admin}</span>
                                                            </div>
                                                            <div className='detail-content-row'>
                                                                <span className='title-name'>User  Paket Wisata</span>
                                                                <span>:</span>
                                                            ...
                                                            <div className='detail-content-row'>
                                                                ...
                                                                <span>:</span>
                                                                {item.npwp_pemilik_paket_wisata ? (
                                                                    <span className='subtitle-name'>{item.npwp_pemilik_paket_wisata}</span>
                                                                ) : (
                                                                    <span className='subtitle-name text-danger'>Undifined</span>
                                                                )}
                                                            </div>
                                                                ...
                                                                <span className='subtitle-name'>
                                                                    {item.detail_admin_verified ? (
                                                                        ...
                                                                        <label className={`badge badge-warning mt-0`}
                                                                            onClick={() => openModalFormVerifikasi(item.id_paket_wisata, item.status_verifikasi)}
                                                                        >Belum diverifikasi</label>
                                                                    )}
                                                                </span>
                                                            </div>
                                                            <div className='detail-content-row'>
                                                                <span className='title-name'>KBLI  Paket Wisata</span>
                                                                <span>:</span>
                                                                {item.kbli_paket_wisata ? (
                                                                    <span className='subtitle-name'>{item.kbli_paket_wisata}</span>
                                                                ) : (
                                                                    <span className='subtitle-name text-danger'>Undifined</span>
                                                                )}
                                                            </div>
                                                            <div className='detail-content-row'>
                                                                <span className='title-name'>NIB  Paket Wisata</span>
                                                                <span>:</span>
                                                                {item.nib_paket_wisata ? (
                                                                    <span className='subtitle-name'>{item.nib_paket_wisata}</span>
                                                                ) : (
                                                                    <span className='subtitle-name text-danger'>Undifined</span>
                                                                )}
                                                            </div>
                                                            <div className='detail-content-row'>
                                                                <span className='title-name'>Alamat  Paket Wisata</span>
                                                                <span>:</span>
                                                                {item.alamat_paket_wisata ? (
                                                                    <span className='subtitle-name'>{item.alamat_paket_wisata}</span>
                                                                ) : (
                                                                    <span className='subtitle-name text-danger'>Undifined</span>
                                                                )}
                                                            </div>
                                                            <div className='detail-content-row'>
                                                                <span className='title-name'>Status  Paket Wisata</span>
                                                                <span>:</span>
                                                                {item.status_paket_wisata ? (
                                                                    <span className='subtitle-name'>Milik {item.status_paket_wisata}</span>
                                                                ) : (
                                                                    <span className='subtitle-name text-danger'>Undifined</span>
                                                                )}
                                                            </div>
                                                            <div className='detail-content-row'>
                                                                <span className='title-name'>NPWP  Paket Wisata</span>
                                                                <span>:</span>
                                                                {item.npwp_paket_wisata ? (
                                                                    <span className='subtitle-name'>{item.npwp_paket_wisata}</span>
                                                                ) : (
                                                                    <span className='subtitle-name text-danger'>Undifined</span>
                                                                )}
                                                            </div>
                                                            <div className='detail-content-row'>
                                                                <span className='title-name'>Harga Tiket  Paket Wisata</span>
                                                                <span>:</span>
                                                                {item.harga_paket_wisata ? (
                                                                    <span className='subtitle-name'>{item.harga_paket_wisata}</span>
                                                                ) : (
                                                                    <span className='subtitle-name text-danger'>Undifined</span>
                                                                )}
                                                            </div>
                                                            <div className='detail-content-row'>
                                                                <span className='title-name'>Kontak  Paket Wisata</span>
                                                                <span>:</span>
                                                                <span className='subtitle-name'>{item.kontak_person_paket_wisata}</span>
                                                            </div>
                                                            <div className='detail-content-row'>
                                                                <span className='title-name'>Total Pengunjung  Paket Wisata</span>
                                                                <span>:</span>
                                                                <span className='subtitle-name'>{item.total_pengunjung_paket_wisata} Pengunjung</span>
                                                            </div>
                                                            <div className='detail-content-row'>
                                                                <span className='title-name'>Kondisi Akses Masuk  Paket Wisata</span>
                                                                <span>:</span>
                                                            ...
                                                            <div className='detail-content-row'>
                                                                <span className='title-name'>Fasilitas  Paket Wisata</span>
                                                                <span>:</span>
                                                                <div className='subtitle-name'>
                                                                    {item.detail_fasilitas_paket_wisata.length !== 0 ? (
                                                                        <>
                                                                            <div className='d-flex flex-column mb-2'>
                                                                                {item.detail_fasilitas_paket_wisata.map((item, index) => {
                                                                                    return (
                                                                                        <span key={index} className='px-1'>- {item.nama_fasilitas_paket_wisata}</span>
                                                                                    )
                                                                                })}
                                                                            </div>
                                                                            <label className={`badge badge-success mt-0`}
                                                                                onClick={() => openModalChangesFasilitas(item.id_paket_wisata, "update")}
                                                                            >
                                                                                Ubah list fasilitas
                                                                            </label>
                                                                        </>
                                                                        ...
                                                                        <label className={`badge badge-warning mt-0`}
                                                                            onClick={() => openModalChangesFasilitas(item.id_paket_wisata, "add")}
                                                                        >Tambahkan list fasilitas</label>
                                                                    )}
                                                                </div>
                                                            </div>
                                                            <div className='detail-content-row'>
                                                                <span className='title-name'>Deskripsi  Paket Wisata</span>
                                                                <span>:</span>
                                                                <span className='subtitle-name' style={{ whiteSpace: 'pre-wrap', width: 850 }}>{item.desk_paket_wisata}</span>
                                                            </div>
                                                            <div className='detail-content-row'>
                                                            <span className='title-name'>Content Paket Wisata</span>
                                                            <span>:</span>
                                                            <div
                                                                className='subtitle-name'
                                                                style={{ whiteSpace: 'pre-wrap', width: 850 }}
                                                                dangerouslySetInnerHTML={{ __html: item.content_paket_wisata }}
                                                            />
                                                            </div>

                                                            <div className='detail-content-row-wisata'>
                                                                <span className='title-name'>Foto Sampul  Paket Wisata</span>
                                                                <span>:</span>
                                                                <img src={item.sampul_paket_wisata} alt={item.sampul_paket_wisata} />
                                                            </div>
                                                            <div className='detail-content-row-wisata'>
                                                                <span className='title-name'>Foto Ruang  Paket Wisata</span>
                                                                <span>:</span>
                                                                {item.ruang_paket_wisata ? (
                                                                    <img src={item.ruang_paket_wisata} alt={item.ruang_paket_wisata} />
                                                                ) : (
                                                                    <span className='subtitle-name text-danger'>Undifined</span>
                                                                )}
                                                            </div>
                                                            <div className='detail-content-row'>
                                                                <span className='title-name'>Author Data  Paket Wisata</span>
                                                                <span>:</span>
                                                            ...
                                                            <div className='detail-content-row'>
                                                                <span className='title-name'>Maps  Paket Wisata</span>
                                                                <span>:</span>
                                                                <span className='subtitle-name'>
                                                                    {item.maps_paket_wisata ? (
                                                                        <label className={`badge ${item.maps_paket_wisata ? " badge-success" : " badge-Warning"} mt-0`}
                                                                            onClick={() => openModalChangesMaps(item.id_paket_wisata, "update", item.maps_paket_wisata)}
                                                                        >
                                                                            {item.maps_paket_wisata ? "Ubah lokasi maps" : "Tambahkan lokasi maps"}
                                                                        </label>

                                                                    ) : (
                                                                        <label className={`badge badge-warning mt-0`}
                                                                            onClick={() => openModalChangesMaps(item.id_paket_wisata, "add")}
                                                            ...
                                                            {/* {item.maps_paket_wisata && (
                                                                <div className='detail-content-row'>
                                                                    <iframe title='Maps' src={item.maps_paket_wisata} width="100%" height="700px" style={{ border: "none" }}></iframe>
                                                                </div>
                                                            )} */}
                                                        </div>
                                                    </div>
                                                ...
                                                <div className='mt-5' style={{ display: activeTab === "Data Pengunjung" ? "" : "none" }}>
                                                    ...
                                                    {DataAjuan.length !== 0 && (
                                                        <>
                                                            <h6 className="card-title">Tabel Data Ajuan Pengunjung {DataDetailPaketWisata[0]?.nama_paket_wisata}</h6>
                                                    ...
                                                    <h4 className="card-title">Tabel Data Pengunjung {DataDetailPaketWisata[0]?.nama_desaWisata}</h4>
                                                    <div className="container-top">
                                                        <button type="button" className="button good rounded" onClick={openModal}>
                                                         ...
                                                         <button 
                                                            ...
                                                            onClick={() => exportDataPengunjung(id_paket_wisata, 'paket_wisata')}
                                                            style={{ padding: "8px 16px", fontSize: "14px", display: "left", alignItems: "left", marginLeft: "-550px" }} 
                                                        >
